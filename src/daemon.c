...
#define PATH_MDM_CUSTOM "/etc/mdm/custom.conf"

#ifndef FALLBACK_MINIMAL_UID
#define FALLBACK_MINIMAL_UID 500
#endif

#define USERDIR LOCALSTATEDIR "/lib/MateAccountsService/users"
...
error_quark (void)
{
  ...
  if (ret == 0)
    {
      ret = g_quark_from_static_string ("mateaccounts_error");
    }
...
daemon_class_init (DaemonClass *klass)
{
        ...
        dbus_g_error_domain_register (ERROR,
                                      "org.freedesktop.MateAccounts.Error",
                                      TYPE_ERROR);
...
reload_autologin_timeout (Daemon *daemon)
{
        ...
        if (!load_autologin (daemon, &name, &enabled, &error)) {
                g_debug ("failed to load mdms custom.conf: %s", error->message);
                g_error_free (error);
                g_free (name);
...
static void
on_mdm_monitor_changed (GFileMonitor      *monitor,
                        GFile             *file,
                        GFile             *other_file,
                        GFileMonitorEvent  event_type,
                        Daemon            *daemon)
...
daemon_init (Daemon *daemon)
{
        ...
        daemon->priv->shadow_monitor = g_file_monitor_file (file,
                                                            ...
                                                            &error);
        g_object_unref (file);
        file = g_file_new_for_path (PATH_MDM_CUSTOM);
        daemon->priv->mdm_monitor = g_file_monitor_file (file,
                                                         G_FILE_MONITOR_NONE,
                                                         NULL,
                                                         &error);
        g_object_unref (file);
        ...
        if (daemon->priv->mdm_monitor != NULL) {
                g_signal_connect (daemon->priv->mdm_monitor,
                                  "changed",
                                  G_CALLBACK (on_mdm_monitor_changed),
                                  daemon);
        } else {
                g_warning ("Unable to monitor %s: %s", PATH_MDM_CUSTOM, error->message);
                g_error_free (error);
       }
...
static gboolean
register_mateaccounts_daemon (Daemon *daemon)
{
...
daemon_new (void)
{
        ...
        if (!register_mateaccounts_daemon (DAEMON (daemon))) {
                    ...
                    DBusGMethodInvocation *context)
{
        ...
        daemon_local_check_auth (daemon,
                                 NULL,
                                 "org.freedesktop.mateaccounts.user-administration",
                                 TRUE,
                ...
                GError     **error)
{
        ...
        result = g_file_set_contents (PATH_MDM_CUSTOM, data, -1, error);
