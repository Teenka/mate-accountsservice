 ...
 *
 * Updates the properties of @user from the mate-accounts service via
                    ...
                    const char *email)
{
        ...
        g_return_if_fail (DBUS_IS_G_PROXY (user->mateaccounts_proxy));

        if (!dbus_g_proxy_call (user->mateaccounts_proxy,
                                "SetEmail",
                                &error,
                                G_TYPE_STRING, email,
                                G_TYPE_INVALID,
 ...
 * @user: the user object to alter.
 * @x_session: an x session (e.g. mate)
 *
