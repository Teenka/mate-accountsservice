...
#define ACCOUNTS_PATH      "/org/freedesktop/MateAccounts"
...
typedef enum {
        ...
        ACT_USER_MANAGER_GET_USER_STATE_ASK_MATEACCOUNTS_SERVICE,
                                 ...
                                 gpointer    user_data)
{
        ...
        if (!manager->priv->is_loaded) {
                g_debug ("ActUserManager: ignoring new user in mateaccounts service with object path %s since not loaded yet", object_path);
                return;
        }

        g_debug ("ActUserManager: new user in mateaccounts service with object path %s", object_path);
                                     ...
                                     gpointer    user_data)
{
        ...
        if (user == NULL) {
                ...
                g_debug ("ActUserManager: tracked user %s removed from mateaccounts service", object_path);
        }
...
static void
find_user_in_mateaccounts_service (ActUserManager                 *manager,
                               ActUserManagerFetchUserRequest *request)
{
        DBusGProxyCall  *call;

        g_debug ("ActUserManager: Looking for user %s in mateaccounts service",
                 request->username);
...
static void
get_mateaccounts_proxy (ActUserManager *manager)
{
        DBusGProxy      *proxy;
         ...
         ActUserManagerFetchUserRequest *request)
{
        ...
        g_debug ("ActUserManager: mateaccount service unavailable, "
                 "giving up");
...
fetch_user_incrementally (ActUserManagerFetchUserRequest *request)
{
        ...
        case ACT_USER_MANAGER_GET_USER_STATE_ASK_MATEACCOUNTS_SERVICE:
...
static void
fetch_user_from_mateaccounts_service (ActUserManager *manager,
                                  ActUser        *user,
                                  const char     *username)
{
        ActUserManagerFetchUserRequest *request;
                              ...
                              GError             **error)
{
        ...
        local_error = NULL;
        res = dbus_g_proxy_call (manager->priv->mateaccounts_proxy,
                                 "CreateUser",
                                 &local_error,
